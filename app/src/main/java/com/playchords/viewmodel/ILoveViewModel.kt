...
class ILoveViewModel(application: Application) : AndroidViewModel(application) {

    private val audioEngine = AudioEngine(application)
    private var playbackJob: Job? = null

    private val _song = MutableStateFlow(ILoveGenerator.generate())
    val song: StateFlow<ILoveSong> = _song
    ...
    fun regenerate() {
        stopPlayback()
        _song.value = ILoveGenerator.generate()
    }
